...
String _uptimeText(double? ratio) => ratio == null ? '—' : '${(ratio * 100).toStringAsFixed(2)}%';
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            ...
            child: Text(monitor.type.toUpperCase(), style: AppTypography.rowType.copyWith(color: t.faint)),
          ),
          SizedBox(
          ...
          SizedBox(width: monitorRowColumnWidths[4], child: _CertCell(monitor: monitor, muted: t.muted)),
        ],
      ),
    );
  }
...
class _RespCell extends StatelessWidget {
  ...
  const _RespCell({required this.monitor, required this.ink, required this.muted});

  @override
  Widget build(BuildContext context) {
    if (monitor.state == MonitorState.pending) {
      return Text('PEND', textAlign: TextAlign.right, style: AppTypography.rowType.copyWith(color: muted));
    }
    if (monitor.state == MonitorState.maintenance) {
      return Text('MAINT', textAlign: TextAlign.right, style: AppTypography.rowType.copyWith(color: muted));
    }
...
class _CertCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Text(text, textAlign: TextAlign.right, style: AppTypography.rowCert.copyWith(color: muted));
  }
}
...
class _DownRow extends StatelessWidget {
  ...
  const _DownRow({required this.monitor, required this.ink, required this.paper});
        ...
        children: [
          ...
          Expanded(
            child: Text(
              ...
              overflow: TextOverflow.ellipsis,
              maxLines: 1,
              style: AppTypography.row.copyWith(color: paper, fontWeight: FontWeight.w700),
            ),
          ),
          SizedBox(
            width: monitorRowColumnWidths[1],
            child: Text(monitor.type.toUpperCase(), style: AppTypography.rowType.copyWith(color: paper)),
          ),
          SizedBox(
            width: monitorRowColumnWidths[2],
            child: Text('DOWN', textAlign: TextAlign.right, style: AppTypography.downLabel.copyWith(color: paper)),
          ),
          SizedBox(
            width: monitorRowColumnWidths[3],
            child: Text(
          ...
          SizedBox(
            ...
            child: certText == null
                ...
                : Text(certText, textAlign: TextAlign.right, style: AppTypography.rowCert.copyWith(color: paper)),
          ),
        ],
      ),
    );
