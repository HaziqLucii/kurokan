...
HostVitals _okVitals({UsageLevel cpuLevel = UsageLevel.ok}) => HostVitals(
      ...
      status: 'running',
      ipv4: '1.2.3.4',
      cpu: Gauge(used: 10, allowed: 100, percentUsed: 10, level: cpuLevel, unit: 'CPU-s'),
      memory: const Gauge(used: 10, allowed: 100, percentUsed: 10, level: UsageLevel.ok, unit: 'MiB'),
      disk: const Gauge(used: 10, allowed: 100, percentUsed: 10, level: UsageLevel.ok, unit: 'MiB'),
      network: const Gauge(used: 1, allowed: 100, percentUsed: 1, level: UsageLevel.ok, unit: 'GiB'),
      processCount: 10,
...
void main() {
  testWidgets('LAST reads --:--:-- before the first fetch resolves', (tester) async {
    await _setWindowSize(tester);
    final monitors = _FakeMonitorSource(() => Completer<List<MonitorStatus>>().future);
  ...
  testWidgets('a down monitor renders in the inverted DOWN row', (tester) async {
    await _setWindowSize(tester);
    final monitors = _FakeMonitorSource(() async => [
          const MonitorStatus(id: '1', name: 'Up Service', type: 'http', state: MonitorState.up),
          const MonitorStatus(id: '2', name: 'Down Service', type: 'http', state: MonitorState.down),
        ]);
  ...
  testWidgets('a crit vitals tile renders inverted with the CRIT label', (tester) async {
    await _setWindowSize(tester);
    final monitors = _FakeMonitorSource(() async => const [
          MonitorStatus(id: '1', name: 'Up Service', type: 'http', state: MonitorState.up),
        ]);
    final vitals = _FakeVitalsSource(() async => _okVitals(cpuLevel: UsageLevel.crit));
  ...
  testWidgets('an error after a value shows the stale footer, not the error footer', (tester) async {
    await _setWindowSize(tester);
    var callCount = 0;
    final monitors = _FakeMonitorSource(() async {
      callCount++;
      if (callCount == 1) {
        return const [MonitorStatus(id: '1', name: 'Up Service', type: 'http', state: MonitorState.up)];
      }
  ...
  testWidgets('manual refresh (⌘R button) is throttled to one trigger per 5 seconds', (tester) async {
    await _setWindowSize(tester);
