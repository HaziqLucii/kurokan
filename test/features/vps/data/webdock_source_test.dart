...
final _serverBody = File(
  'test/fixtures/webdock_server.json',
).readAsStringSync();
final _metricsBody = File(
  'test/fixtures/webdock_metrics_now.json',
).readAsStringSync();

WebdockSource _sourceWith(http.Client client) =>
    WebdockSource(slug: 'demo-server', apiToken: 'wd_secret', client: client);
...
void main() {
  test(
    'sends a Bearer token to both endpoints and parses HostVitals',
    () async {
      ...
      expect(
        requestedPaths,
        containsAll([
          '/v1/servers/demo-server',
          '/v1/servers/demo-server/metrics/now',
        ]),
      );
  ...
  test(
    'throws AuthError on 401 from the metrics endpoint even when the server endpoint is 200',
    () async {
  ...
  test(
    'throws AuthError on 403 (token valid but missing read:servers scope)',
    () async {
  ...
  test(
    'throws HttpError on 429 (no special-casing; the spec never names one)',
    () async {
  ...
  test(
    'throws ParseError when a required field is missing from the response',
    () async {
      final client = MockClient((request) async {
        if (request.url.path.endsWith('/metrics/now')) {
          return http.Response(_metricsBody, 200);
  ...
  test(
    'throws ParseError when a required field is missing from the metrics response',
    () async {
      final client = MockClient((request) async {
        if (request.url.path.endsWith('/metrics/now')) {
  ...
  test(
    'processCount is null (not zero) when the processes block is absent',
    () async {
      final client = MockClient((request) async {
        if (request.url.path.endsWith('/metrics/now')) {
          return http.Response(
            _metricsBody.replaceAll('"processes"', '"_processes"'),
            200,
          );
        }
        return http.Response(_serverBody, 200);
  ...
  test(
    'throws ParseError on a non-JSON body (e.g. wrong host answering with HTML)',
    () async {
