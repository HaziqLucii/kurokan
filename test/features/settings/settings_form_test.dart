...
Widget _harness({required AppConfig? initial, required String? Function(AppConfig) onSave}) {
  return MaterialApp(
    theme: buildAppTheme(Brightness.dark),
    home: Scaffold(body: SettingsForm(initial: initial, onSave: onSave)),
  );
}
...
void main() {
  testWidgets('create mode: blank required fields blocks save with an error', (tester) async {
    AppConfig? saved;
    await tester.pumpWidget(_harness(initial: null, onSave: (c) { saved = c; return null; }));
  ...
  testWidgets('create mode: blank kuma url blocks save with an error', (tester) async {
    AppConfig? saved;
    await tester.pumpWidget(_harness(initial: null, onSave: (c) { saved = c; return null; }));
  ...
  testWidgets('create mode: filling every field calls onSave with the typed values', (tester) async {
    AppConfig? saved;
    await tester.pumpWidget(_harness(initial: null, onSave: (c) { saved = c; return null; }));

    await tester.enterText(_fieldAt(0), 'my-slug'); // webdock slug
    await tester.enterText(_fieldAt(1), 'wd_token'); // webdock token
    await tester.enterText(_fieldAt(2), 'https://kuma.example.tld'); // kuma url
    await tester.enterText(_fieldAt(3), 'uk1_key'); // kuma api key
    await tester.enterText(_fieldAt(4), '60'); // poll interval
  ...
  testWidgets('edit mode: blank secret fields keep the existing secrets', (tester) async {
    ...
    AppConfig? saved;
    await tester.pumpWidget(_harness(initial: initial, onSave: (c) { saved = c; return null; }));
  ...
  testWidgets('edit mode: typing a new secret overrides the existing one', (tester) async {
    const initial = AppConfig(
      webdock: WebdockConfig(slug: 'old-slug', apiToken: 'old-token'),
      kuma: KumaConfig(url: 'https://old.kuma.tld', apiKey: 'old-key'),
    );
