...
void main() {
  ...
  group('against the real captured fixture', () {
    final body = File('test/fixtures/uptime_kuma_metrics.txt').readAsStringSync();
    final monitors = parser.parse(body);

    test('extracts exactly the monitor_* rows, ignoring process_*/nodejs_*/app_version', () {
    ...
    test('joins families by the full label set (this Kuma instance has no monitor_id label)', () {
    ...
    test('uptime24h is null when monitor_uptime_ratio is absent from the body', () {
  ...
  group('synthetic edge cases', () {
    test('unescapes \\" and \\\\ in label values and joins by monitor_name', () {
      const body = '''
...
''';
    ...
    test('a Kuma instance with zero monitors but real process_*/nodejs_* lines returns an empty list', () {
      const body = '''
# HELP process_cpu_seconds_total Total CPU time
# TYPE process_cpu_seconds_total counter
process_cpu_seconds_total 123.45
''';
    ...
    test('throws FormatException on a body with no recognizable Prometheus lines at all', () {
    ...
    test('keeps duplicate monitor names as separate rows when their label sets differ', () {
      const body = '''
...
''';
      ...
      expect(monitors.map((m) => m.state).toSet(), {MonitorState.up, MonitorState.down});
